...
local function decode_scanArray(s,startPos)
  ...
  repeat
    ...
    assert(startPos<=stringLen, 'JSON String ended unexpectedly scanning array.')
    object, startPos = util.JSONToTable(s,startPos)
...
local function decode_scanObject(s,startPos)
  ...
  repeat
    ...
    -- Scan the key
    key, startPos = util.JSONToTable(s,startPos)
    ...
    startPos = decode_scanWhitespace(s,startPos+1)
    assert(startPos<=stringLen, 'JSON string ended unexpectedly searching for value of key ' .. key)
    value, startPos = util.JSONToTable(s,startPos)
    object[key]=value
...
function util.TableToJSON (v)
  ...
  if vtype=='table' then
    ...
    if bArray then
      for i = 1,maxCount do
        table.insert(rval, util.TableToJSON(v[i]))
      end
    else    -- An object, not an array
      for i,j in pairs(v) do
        if isEncodable(i) and isEncodable(j) then
          table.insert(rval, '"' .. encodeString(i) .. '":' .. util.TableToJSON(j))
        end
      end
    end
    if bArray then
...
function util.JSONToTable(s, startPos)
  ...
  if string.sub(s,startPos,startPos+1)=='/*' then
    return util.JSONToTable(s, decode_scanComment(s,startPos))
  end
